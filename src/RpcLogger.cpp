...
void RpcLogger::log_fatal(string msg)
{
    set_level(FATAL);
    log(msg);
    std::cout << "Force Majeure is being dealt with" << std::endl;;
    //sleep(10);
    std::cout << "Question has been record in log file, please restore system,bye" << std::endl;
    exit(0);
}
...
RpcLogger::RpcLogger()
{
    thread wirte_thread([&]() {
        for (;;)
        {
            ...
            {
                //打印muduo错误日志
                std::cout << "logger file: " << file_name << " opne error" << std::endl;
            }
